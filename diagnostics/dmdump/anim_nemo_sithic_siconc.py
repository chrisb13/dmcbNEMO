...
for k in range(5475, len(ifiles)+1, 5):
    print ifiles[k]

    # Load the velocity fields and remask them.
    sithic = nemo.load_field('sithic', '', '', ifiles[k], 'T')
    sithic = nemo.mask_field(sithic, tmask)
    siconc = nemo.load_field('siconc', '', '', ifiles[k], 'T')
    siconc = nemo.mask_field(siconc, tmask)
    sithic = sithic * siconc
    ...
    cs_ice = map.contourf(glamt.T, gphit.T, np.squeeze(sithic).T,
                          np.arange(0., 10.25, 0.25),
                          latlon='true', cmap='rainbow', extend='max')

    # Add a colour bar.
    cbar = map.colorbar(cs_ice, 'right', ticks=np.arange(0., 12.5, 2.5),
                        pad=0.5)
    ...
    plt.savefig(''.join(['../figs/',
                         nemodir[-29:-1], '/anim/sithic_siconc/sithic_siconc_', str(k).zfill(5), '.png']),
