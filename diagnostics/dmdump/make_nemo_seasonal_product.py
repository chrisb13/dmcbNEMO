...
for k in season_to_ave:
    field = nemo.calc_seasonal_product(field1, field2, tmask, 'T',
                                       homedir, nemodir, 'd01/I/',
    ...
    if k is not 'DJF':
        np.save(''.join([homedir, nemodir, 'POST/', field1, '_', field2, '_', k, '_', str(init_year),
                         '-', str(init_year + num_years - 1)]), field)
    else:
        np.save(''.join([homedir, nemodir, 'POST/', field1, '_', field2, '_', k, '_', str(init_year-1),
                         '-', str(init_year + num_years - 1)]), field)
