...
if save_output:
    ...
    meansss2 = np.ndarray(shape=[len(tfiles), 2])
    ...
    for k in range(len(tfiles)):
        ...
        meansss2[k, 0] = tfiles[k].split('/ORCH0083-LIM3_', 1)[1].split('_T', 1)[0]
        meansss2[k, 1] = (sss*sss*area).sum() / area.sum()

# --------------------------------------------------------------------------- #
# Spit the numbers out to file, if requested.
if save_output:
    np.save(''.join([homedir, nemodir, 'TIDY/ARCHIVE/POST/meansss2']), meansss2)

# --------------------------------------------------------------------------- #
# If we're not saving the number, we're loading them.
if not save_output:
    meansss2 = np.load(''.join([homedir, nemodir, 'TIDY/ARCHIVE/POST/meansss2.npy']))
...
p = plt.plot(np.linspace(0, 5*len(meansss2), len(meansss2)+1),
             np.concatenate(([0.], meansss2[:, 1]), axis=0),
             '-b', linewidth=3)
ax = plt.gca()
ax.set_aspect(5*len(meansss2)/280.0)


plt.axis([0, 5*len(meansss2), 1090, 1370.0])
plt.xticks(np.arange(0, 5*len(meansss2)+1, 365))
plt.yticks(np.linspace(1090., 1370.0, 5))
plt.xlabel('Days', fontname='arial', fontsize=30, fontweight='bold')
plt.ylabel('Ave. SSS^2 ($\mathrm{g^2/kg^2}$)',
...
plt.savefig(''.join(['../figs/', nemodir[-29:-1], '/meansss2_timeseries.pdf']), bbox_inches='tight')
