...
tmask = np.squeeze(nemo.load_field('tmask', homedir, nemodir, gridfile, 'T')[1750:1751, :, :])
...
for k in range(0, len(tfiles)+1, 1):
    ...
    age = np.squeeze(nemo.load_field('voagee3t', '', '', tfiles[k], 'T')[1750:1751, :, :])
    age = nemo.mask_field(age, tmask)
    e3t = np.squeeze(nemo.load_field('e3t', '', '', tfiles[k], 'T')[1750:1751, :, :])
    ...
    cs_age = plt.contourf(np.repeat(gphit[1750:1751, :].T, 75, axis=1),
    ...
    plt.xlabel('Latitude', fontname='arial', fontsize=8, fontweight='bold')
    plt.ylabel('Depth ($\mathrm{m}$)', fontname='arial', fontsize=8, fontweight='bold')
    ...
    plt.savefig(''.join(['../figs/',
                         nemodir[-29:-1], '/anim/age_pac/age_pac_', str(k).zfill(5), '.png']),
