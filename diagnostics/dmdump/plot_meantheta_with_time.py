...
if save_output:
    ...
    for k in range(len(tfiles)):
        ...
        meantheta[k, 0] = tfiles[k].split('/ORCH0083-LIM3_', 1)[1].split('_T', 1)[0]
        meantheta[k, 1:] = np.sum(theta*area, axis=(0, 1)) / np.sum(e3t*area, axis=(0, 1))

# --------------------------------------------------------------------------- #
# Spit the numbers out to file, if requested.
if save_output:
    np.save(''.join([homedir, nemodir, 'TIDY/ARCHIVE/POST/meantheta_m01']), meantheta)

# --------------------------------------------------------------------------- #
# If we're not saving the number, we're loading them.
if not save_output:
    meantheta = np.load(''.join([homedir, nemodir, 'TIDY/ARCHIVE/POST/meantheta_m01.npy']))
...
p = plt.plot(np.linspace(15.0, 30.0*len(meantheta)-15.0, len(meantheta))/365.0,
             meantheta[:, 1], '-b',
             np.linspace(15.0, 30.0*len(meantheta)-15.0, len(meantheta))/365.0,
             meantheta[:, 17], '-g',
             np.linspace(15.0, 30.0*len(meantheta)-15.0, len(meantheta))/365.0,
             meantheta[:, 30], '-r',
             np.linspace(15.0, 30.0*len(meantheta)-15.0, len(meantheta))/365.0,
             ...
             linewidth=1)
ax = plt.gca()
ax.set_aspect(40.0/15.0)

plt.axis([0, 40.0, 0, 15.0])

plt.xticks(np.arange(0, 41.0, 5.))
ax.set_xticks(np.arange(0, 41.0, 1.), minor=True)

plt.yticks(np.linspace(0., 15.0, 5))
...
plt.savefig(''.join(['../figs/', nemodir[-29:-1], '/meantheta_m01_timeseries.pdf']), bbox_inches='tight')
