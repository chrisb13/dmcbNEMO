...
if save_output:
    ...
    for k in range(len(tfiles)):
        ...
        meansss[k, 0] = tfiles[k].split('/ORCH0083-LIM3_', 1)[1].split('_T', 1)[0]
...
if not save_output:
    meansss = np.load(''.join([homedir, nemodir, 'TIDY/ARCHIVE/POST/meansss.npy']))
...
p = plt.plot(np.linspace(0, len(meansss), len(meansss)+1)/365.0,
             np.concatenate(([0.], meansss[:, 1]), axis=0),
             '-b', linewidth=1)
ax = plt.gca()
...
plt.ylabel('Ave. SSS ($\mathrm{g/kg}$)',
...
plt.savefig(''.join(['../figs/', nemodir[-29:-1], '/meansss_timeseries.pdf']), bbox_inches='tight')
